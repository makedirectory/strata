...
The save/load path is entirely browser-local; the server routes that _are_ live
handle discovery and the interest signal; the `/api/graphs` tier is built and
tested but intentionally off the save path, waiting for a durable backend:

```mermaid
flowchart TB
  subgraph activep["Active save path (browser)"]
    hook["useFlow"] --> ls["localStore.ts"]
    ls --> storage["localStorage<br/>strata:graphs:v1"]
    hook --> snaps["snapshots.ts<br/>strata:snapshots:v1"]
  end

  subgraph livep["Live server routes (in use)"]
    discr["/api/discover (+ gcp · azure)"]
    interestr["/api/interest"]
  end

  subgraph retainedp["Retained server tier — not on the save path"]
    routes["/api/graphs Route Handlers"] --> repo["Repository interface"]
    repo --> file["FileRepository (default)"]
    repo -.-> pg["PostgresRepository (designed-for)"]
    repo -.-> dyn["DynamoRepository (designed-for)"]
  end

  hook -.->|"live cloud scan"| discr
```
...
surfaces its own quota error (_"Browser storage is full — delete some saved
versions."_), and is intentionally **not cross-device** (promotable later to a
`Repository` `versions` sub-resource — see [Roadmap §1](/docs/architecture/roadmap)).
...
- Discovery credentials never reach the browser store. AWS keys entered in the
  Connect dialog live only in transient React state, are sent to `/api/discover`
  over HTTPS for one scan, and are discarded server-side; GCP/Azure use the
  server's ambient credentials and nothing is entered at all. The
  no-credentials-in-graph invariant (see
  [Live Discovery & MCP](/docs/architecture/mcp-integration#credential-boundary-non-negotiable))
  means a saved diagram can't contain a secret either — config is filtered to
  registry-known fields.
- So **there's nothing credential-shaped to encrypt.** What's persisted is the
  user's own architecture diagram (resource types, names, config, relationships) —
  comparable to a local file they created.
...
> persistence. The server routes the UI _does_ still use are **`/api/discover`**,
> **`/api/discover/gcp`**, and **`/api/discover/azure`** (live cloud discovery) and
> **`/api/interest`** (below). The rest of this page documents that retained server
> tier, which is where a durable backend will plug in.

### "Coming soon" interest signal (`/api/interest`)

A tiny, datastore-free notification path — **not** part of the retained tier. When
`NEXT_PUBLIC_STRATA_HOSTED=1`, the "I'd use this" button (`recordInterest` in
`src/lib/interest.ts`) POSTs `{ feature, note? }` to `src/app/api/interest/route.ts`
(skipped locally — nothing to collect on a single-user box). The route validates
the small payload, **always `console.log`s** it (captured in the host's function
logs, e.g. Vercel), and — when **`STRATA_INTEREST_WEBHOOK`** is set — forwards a
one-line message to that incoming webhook (Slack `text` + Discord `content` in one
body; ntfy.sh / a webhook→email relay also work). No persistence, no credentials,
no npm dependency; a webhook failure never affects the user. To start receiving
them, set `STRATA_INTEREST_WEBHOOK` in the hosted env.
