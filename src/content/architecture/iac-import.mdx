...
- `exportTerraform(graph)` → `{ hcl, report }`
- `exportIaC(graph, format)` → `{ content, filename, report }`

Join keys run in reverse: CloudFormation uses each service's `cfnType` directly;
Terraform uses `SERVICE_ID_TO_TF_TYPE`, the inverse of `TF_TYPE_TO_SERVICE_ID`
(first-listed wins as the canonical type for many-to-one services). Output is
**deterministic** — resources are id-sorted, so the same graph always serialises
identically (important for tests).
...
- Required `configFields` that aren't set are emitted as `TODO` placeholders and
  tallied in `todos`.
- Containment + typed edges coarsen to `DependsOn` / `depends_on` (a documented
  delta — structure is preserved, edge _kind_ is not).

A **round-trip stability** test asserts that
`importIaC(exportCloudFormation(g))` preserves resource types and edge structure
for services whose `cfnType` resolves uniquely (values may differ — lossy is
expected).
