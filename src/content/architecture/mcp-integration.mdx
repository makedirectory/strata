...
| `list_repo_roots`       | `{ path }`                           | `detectRepoRoots` (Terraform root modules under a local path)                                           |
| `connect_repo`          | `{ path, root?, roots?, strategy? }` | `connectRepo` (map a local Terraform repo; see [Connecting a Repository](/docs/guide/connecting-repos)) |
...
The one exception is `connect_repo` / `list_repo_roots`: these read the local
filesystem and may shell out to `terraform`/`tofu` (via `src/server/connectRepo.ts`),
so they are **local-only** and not pure. They still use **no cloud credentials** and
never mutate the source repo — the graph-building itself is delegated to the pure
`importHclJson` / `importTerraform` engines. Because a tool can now do async work,
`handleMcpMessage` is `async` and stdout writes are serialised to preserve order.
