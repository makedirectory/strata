...
| `import_iac`            | `{ content \| path, name? }`            | `importAnyIaC` ([IaC import](/docs/architecture/iac-import)); returns a `graphId` handle |
| `merge_graphs`          | `{ graphs: [{name, graph}], wrapAs? }`  | `combineGraphs` (fold N per-account graphs onto one canvas)                              |
...
| `export_iac`            | `{ graph \| graphId, format }`          | `exportIaC` (CloudFormation / Terraform)                                                 |
| `estimate_cost`         | `{ graph \| graphId, assumptions? }`    | `estimateMonthlyCost` / `estimateTotal` (returns `isFloor` + unmapped billable types)    |

**Large state, small context.** `import_iac` accepts a local `path` (read
server-side — `terraform show -json` is often multi-MB) and returns a summary
plus a **`graphId`** handle from a non-persistent, in-process registry. The
graph-consuming tools (`estimate_cost`, `review_account`, `export_iac`,
`validate_architecture`) accept that `graphId` in place of an inline `graph`, so
a multi-hundred-resource estate never has to round-trip through the agent's
context. `estimate_cost` also takes optional `assumptions` (`regionMultiplier` /
`hoursPerMonth` / `discountPct`) and always reports whether the total is a
**floor** (some billable types unpriced). See the
[Multi-Account Cost Model](/docs/guide/multi-account-cost) recipe.
