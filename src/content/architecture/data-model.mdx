...
- **`Account`** — an account in scope: `accountId` (an AWS 12-digit account id, or
  a GCP project / Azure subscription identifier), `name`, optional `environment`
  (`"prod" | "staging" | …`) and `color`.
...
- **`ResourceInstance`** — a concrete instance of a service:
  ...
  - `position?: CanvasPosition` (`x, y, w, h`) — presentation kept separate from
    data.
  - `raw?: RawSource` — the verbatim source `type` + `properties` from an IaC
    import, kept so the resource can be re-emitted faithfully (never holds
    credentials). See [IaC Import & Export](/docs/architecture/iac-import#lossless-sidecar-faithful-re-emit).
...
- **`InfrastructureGraph`** — the top-level persisted entity: `id`, `name`,
  ...
  `createdAt`/`updatedAt` (stamped by the repository, never by scripts),
  `schemaVersion` (`SCHEMA_VERSION = 1`, for forward migration), and optional
  `iacSource` — the template-level sections (Parameters / Mappings / Conditions /
  Outputs / Metadata) preserved from an IaC import for faithful re-emit (see
  [IaC Import & Export](/docs/architecture/iac-import#lossless-sidecar-faithful-re-emit)).
...
The graph is the aggregate root; resources self-reference through `parentId` for
containment and are wired together by typed `Relationship` edges:

```mermaid
erDiagram
  InfrastructureGraph ||--o{ Account : "accounts[]"
  InfrastructureGraph ||--o{ ResourceInstance : "resources[]"
  InfrastructureGraph ||--o{ Relationship : "relationships[]"
  Account ||--o{ ResourceInstance : "accountId"
  ResourceInstance ||--o{ ResourceInstance : "parentId (containment)"
  ResourceInstance ||--o{ Relationship : "from / to"

  InfrastructureGraph {
    string id
    string name
    int schemaVersion
    IacSource iacSource "lossless re-emit"
  }
  Account {
    string accountId "AWS id / GCP project / Azure sub"
    string environment
  }
  ResourceInstance {
    string id
    string serviceId "→ ServiceDefinition"
    string accountId
    string region
    string parentId "containment tree"
    json config "keyed by ConfigField"
    string source "manual | imported | mcp"
    RawSource raw "verbatim IaC, never creds"
  }
  Relationship {
    string id
    string from "ResourceInstance.id"
    string to "ResourceInstance.id"
...
> **Note:** containment is both _modeled_ here and _rendered_ — children pack
> visually inside their parent's bounds via the pure layout engine in
> `src/canvas/layout.ts`. See [Visual Mapping](/docs/architecture/visual-mapping).

## Placement scopes

`ServiceDefinition.scope` (`global | region | az | vpc | subnet | project |
resource-group`) describes where a resource conceptually belongs and is the basis
...
- `validateGraph(graph)` returns structural errors: duplicate resource ids,
  ...
  [Persistence](/docs/architecture/persistence)) and it is covered by the model and
  route tests in CI (see [Testing](/docs/architecture/testing)).
