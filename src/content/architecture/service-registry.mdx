...
validation engine, and MCP/IaC importers all read from it — nothing about a
service is hardcoded elsewhere.
...
- `cfnType` — the CloudFormation type, which **doubles as the MCP / Cloud Control
  import discriminator** (see [MCP Integration](/docs/architecture/mcp-integration)).
...
`allServices`, `servicesByCategory`, `serviceColor` (per-service override →
category colour fallback), `serviceIcon`, `defaultConfig` (builds the initial
config object from `ConfigField.default`s), and `searchServices` (free-text over
name / fullName / description / id / keywords).
...
**This should be wired into CI** (see [Testing](/docs/architecture/testing) and
