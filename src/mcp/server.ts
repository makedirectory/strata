...
export const TOOLS: McpTool[] = [
  ...
  },
  {
    name: "connect_repo",
    description:
      "Build a Strata graph from a local Terraform/OpenTofu repository, one Account layer per root. Auto-ladder: resolves modules via `terraform plan` when a binary is available (no cloud credentials — throwaway copy, never mutates the repo), else a static HCL parse. `strategy`: auto|static|resolved. `root`/`roots` select roots (default all). Local only.",
    inputSchema: objectSchema(
      {
        path: { type: "string" },
        root: { type: "string" },
    ...
    run: async (a) => {
      ...
      const roots = Array.isArray(a.roots)
        ...
        : str(a, "root")
          ? [str(a, "root")!]
          : undefined;
      const r = await connectRepo(path, {
        roots,
        strategy: str(a, "strategy") as ConnectStrategy | undefined,
      });
      return {
        roots: r.roots,
        resourceCount: r.graph.resources.length,
  ...
  },
  {
    name: "import_plan",
    description:
      "Visualise a Terraform/OpenTofu plan as a graph + change diff (create/update/delete/replace). Pass `planJson` (the text of `terraform show -json <planfile>` — no credentials), OR `repoPath` to run `plan` locally in that repo (uses its backend + ambient credentials; local only).",
    inputSchema: objectSchema({
    ...
    run: async (a) => {
      const planJson = str(a, "planJson");
      const repoPath = str(a, "repoPath");
      const r = planJson
        ? importPlanJson(planJson)
        : repoPath
          ? await runRepoPlan(repoPath, { root: str(a, "root") })
          : null;
      if (!r) throw new Error("Provide `planJson` or `repoPath`.");
      return {
        resourceCount: r.graph.resources.length,
        counts: r.diff.counts,
...
export function runStdio(): void {
  ...
  process.stdin.on("data", (chunk: string) => {
    ...
    while ((nl = buffer.indexOf("\n")) >= 0) {
      ...
      // Serialise handling so async tools (connect_repo / import_plan) can't
      // interleave their responses out of request order on the single stdout.
